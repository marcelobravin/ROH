...
function calcula_frete ($servico,$cep_origem,$cep_destino,$peso,$mao_propria,$valor_declarado,$aviso_recebimento, $comprimento, $altura, $largura, $diametro)
{
	...
	# FALHAS============================================
		...
		$retorno = identificraErro( abs((int) $frete->Erro) );
	}

	return $retorno;
}

function identificraErro ($erro)
{
	switch ( $erro ) {
		...
		default: $retorno = identificarCErro($erro);
	}
	return $retorno;
}

function identificarCErro ($erro)
{
	switch ( $erro ) {
		...
		default: $retorno = "Erro no cálculo do frete, código de erro: ". $erro;
	}
	return $retorno;
}
